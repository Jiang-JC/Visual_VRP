...
def tsp_solve(input_data, Greedy=True):
    # node_ = np.loadtxt('./test_data/tsp100_test_data.csv', dtype=np.float32, delimiter=',')
    ...
    for i, (x1, y1) in enumerate(input_data):
        for j, (x2, y2) in enumerate(input_data):
    ...
    for i in range(1):
        ...
        data = Data(x=torch.from_numpy(input_data).float(), edge_index=edges_index,
    ...
    # print(tour)
    # plot_tsp(input_data, tour.squeeze().cpu().numpy(), ax,202,Greedy)
...
# input_data = np.random.rand(20, 2)\
# tsp_solve(input_data, Greedy=True)